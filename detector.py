...
def separator(ngram_generator, concepts, window):
    '''
...
def detector(s, concepts, stopwords, sep='_'):
    r = list(separator(ngram([s], 15, sep), concepts, 15))[0]
    return [x for x in r if x in concepts and x not in stopwords]
...
def __detect_concepts(input_file, output_file, concepts, window_size):
    ...
    d = separator(ngram(xutils.open_file(input_file), window_size), concepts, window_size)
...
if __name__ == '__main__':
    ...
    if sys.argv[1]=='detect':
        ...
        window_size = 15
        concepts = set(xutils.load('umls_concepts.txt'))
    ...
    if sys.argv[1]=='count':
        ...
        for frq in freq:
            for c,f in frq.items():
                fr[c] = fr[c] + f
        xutils.write_all_lines(xutils.Dirs.Temp + sys.argv[3] + '.tsv',
